...
def inference(model_path_l, model_path_r, test_loader):
    ...
    for minibatch in tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            label_exist = model_CLS(imgs)
            # import pdb;pdb.set_trace()
            label_exist = label_exist.argmax(dim=-1)
            label_exist = int(label_exist)
            imgs_l = wandb.Image(logits_l[0])
            imgs_r= wandb.Image(logits_r[0])
            wandb.log({'img_l: ': imgs_l, 'img_r: ': imgs_r})
        ...
        #     pred[:, 3, :, :] = 1
        # else:
        #     print('the label_exist is wrong')
        #     assert (False)
        pred = (sof_l + sof_r) / 2
        pred = pred.cpu().numpy()    
...
def main():
    ...
    inference(model_path_l, model_path_r, test_loader)
    move()
    # TO3D()
